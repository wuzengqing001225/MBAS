...
def find_record_folders(base_path: str, model: str) -> List[str]:
    record_folders = []
    for root, dirs, files in os.walk(base_path):
        for folder in dirs:
            if re.match(r'^Record_\d{6}_\d{6}' + f'_{model}.*$', folder):
...
def wrap_text(text: str, width: int = 100) -> str:
    ...
    for char in text:
        ...
        if current_line_length >= width and not in_tag:
...
def generate_diff_html(json1: Dict, json2: Dict) -> str:
    ...
    for tag, i1, i2, j1, j2 in s.get_opcodes():
        if tag == 'equal':
            for line in json1_lines[i1:i2]:
                result.append(wrap_text(line))
        elif tag == 'replace':
...
def generate_html_report(differences: List[Dict[str, str]], output_file: str) -> None:
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write('<html><body><h1 style="font-family: Consolas;">Prompt History Comparison</h1>')
...
def main(base_path: str, output_file: str, model: str) -> None:
    record_folders = find_record_folders(base_path, model)
    differences = []
...
if __name__ == "__main__":
    sys.path.append(".")
    base_path = './output'
    model = 'gpt-4-0314' #'claude-3'
    output_file = f'./AnalysisModule/results/prompt_history_comparision_{model}.html'
    main(base_path, output_file, model)
