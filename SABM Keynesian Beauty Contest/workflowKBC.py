...
def simulation(configEnv):
    ...
    ## Plot results
    if configEnv["verbosity"] >= 0: print(keyResults["variance"])
    # plotPath = f"{pathOutputFolder}/variance.pdf"
    plotPath = f"{pathOutputFolder}/variance.png"
    plotVariance(keyResults["variance"], plotPath, varEnv)

    display.display_results(0)
    return keyResults, plotPath

# Single run
if __name__ == "__main__":
    from configEnv import configEnv
    keyResults, plotPath = simulation(configEnv)
